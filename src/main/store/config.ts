...
export const config = {
  ...
  getWatchedProjectIds(): string[] {
    return (configStore as any).get('watchedProjectIds', []);
  },

  setWatchedProjectIds(projectIds: string[]): void {
    (configStore as any).set('watchedProjectIds', projectIds);
  },

  addWatchedProject(projectId: string): void {
  ...
  removeWatchedProject(projectId: string): void {
    const projectIds = this.getWatchedProjectIds();
