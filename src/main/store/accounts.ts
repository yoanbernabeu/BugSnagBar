...
import { BugsnagAccount } from '../../shared/types/account';
import { keychain } from './keychain';

interface AccountsStore {
  accounts: BugsnagAccount[];
}
...
export const accounts = {
  getAll(): BugsnagAccount[] {
    return (accountsStore as any).get('accounts', []);
  },

  getById(id: string): BugsnagAccount | undefined {
    const allAccounts = this.getAll();
    return allAccounts.find((account) => account.id === id);
  },

  getActive(): BugsnagAccount[] {
    return this.getAll().filter((account) => account.isActive);
  },

  add(account: Omit<BugsnagAccount, 'id'>, token: string): BugsnagAccount | null {
    const id = uuidv4();
    const newAccount: BugsnagAccount = {
      ...account,
      id,
    };
  ...
  update(id: string, updates: Partial<Omit<BugsnagAccount, 'id'>>): BugsnagAccount | null {
    const allAccounts = this.getAll();
  ...
  setActive(id: string, isActive: boolean): BugsnagAccount | null {
